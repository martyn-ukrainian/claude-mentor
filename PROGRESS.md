...
**Поточна фаза:** Фаза 0 ✅ **completed** → Фаза 1 (математика) **на паузі, 6/8 уроків** → **Фаза 2 ✅ completed, 9/9** → **Фаза 3 (Deep Learning) — старт 2026-08-09, Уроки 01-09 пройдено (9/12)**

**Рішення від 2026-08-13 (Урок 09 Фази 3 ЗАКРИТО — механіка self-attention, та сама сесія що Уроки 06-08):** Q/K/V, scaled dot-product attention (`softmax(QKᵀ/√d)V`), positional encoding — на конкретному 2-токенному прикладі вручну + робоча PyTorch-реалізація (`SelfAttention`, перевірена на shape `(1,3,4)`). **Нова методологічна знахідка:** перший явний запит студента на візуалізацію в курсі — текстове пояснення (навіть з числами) не зайшло, студент попросив "візуально", сам аргументовано покращив мою першу (плоску 2D) пропозицію до об'ємної (3D, висота=score, обертається мишкою) і мав рацію. Сильне самостійне питання про паралелізм attention vs RNN (сам намацав ключову архітектурну відмінність). Feynman закрився з другої спроби, одна неточність (V нібито обчислюється з Q/K, а не незалежно) записана в voice-retrieval-queue. Деталі — `memories/lessons/04-09-attention.md`, студентський урок — `docs/phase-3-deep-learning/09-attention.md`. Код: `code/phase-3/09_attention.py`. **Наступний крок — Урок 10 Фази 3, multi-head attention + практичний проект — окрема свіжа сесія** (тема залишається найважчою в Фазі 3, marathon 06→07→08→09 вже довгий).

**Рішення від 2026-08-13 (Урок 08 Фази 3 ЗАКРИТО, та сама сесія що Уроки 06-07):** RNN/LSTM — за ROADMAP свідомо менший обсяг ("коротко, як історія"). Іграшкова задача (RNN вчиться рахувати суму послідовності через hidden state) — модель узагальнила на новий приклад (передбачення ≈15 проти справжніх 15). LSTM показаний концептуально (vanishing gradient по часових кроках, gates) + shape-demo, без окремого повного проекту — студент сам перевірив заміну RNN→LSTM з власної цікавості. Дорогий баг уроку — `dim=0` замість `dim=1` при підсумовуванні мітки (astronomічний loss), новий конкретний приклад давнього боргу axis-семантики. Деталі — `memories/lessons/04-08-rnn-lstm.md`, студентський урок — `docs/phase-3-deep-learning/08-rnn-lstm.md`. Код: `code/phase-3/08_rnn_lstm.py`. **Наступний крок — Урок 09 Фази 3, Transformers/attention** (за ROADMAP позначені "обов'язково") — ймовірно суттєво важча тема, варто окремої сесії, не продовжувати в тому самому марафоні.

**Рішення від 2026-08-13 (Урок 07 Фази 3 ЗАКРИТО, та сама сесія що Урок 06):** Регуляризація в DL — dropout, batch norm, early stopping, нашаровані послідовно на CNN з Уроку 06. Фінальний результат **98.82% test / 98.95% train** (кращий за Урок 06). Стабільний самостійний перенос патерну "клас потребує `()`, число в `__init__` не у виклику" (двічі поспіль, ReLU/Dropout → BatchNorm). Early stopping — перша тема Фази 3 де семантика зійшлась з механікою з першої спроби на Feynman; dropout і batch norm обидва мали помилки на Feynman, закриті прямим поясненням, записані в voice-retrieval-queue. Сам, з власної ініціативи, збільшив кількість епох щоб побачити спрацювання early stopping. Реалістичний негативний результат (dropout не дав ефекту на цій малій CNN) прийнятий без форсування висновку. Деталі — `memories/lessons/04-07-regularization.md`, студентський урок — `docs/phase-3-deep-learning/07-regularization.md`. Код: `code/phase-3/07_regularization.py`. **Наступний крок — Урок 08 Фази 3**, звірити з ROADMAP.md (далі: RNN/LSTM коротко, або Transformers/attention — позначені як "обов'язково").

**Рішення від 2026-08-13 (Урок 06 Фази 3 ЗАКРИТО):** Retrieval-розігрів (`retrieval_weight_sharing.py`) — weight-sharing chain rule вперше стабілізувався символьно (без числової похибки як єдиного робочого шляху), третій прохід теми. Feynman-питання ("чому CNN з ~19x меншою кількістю параметрів дала кращий результат за Linear MLP") закрито через розпад на дві частини + студент сам провів крос-доменну аналогію до Random Forest voting (новий тип сигналу — самостійна аналогія з іншої фази). Записано `docs/phase-3-deep-learning/06-cnn-mnist.md`, `memories/profile.md`, `memories/lessons/04-06-cnn-mnist.md`. **Наступний крок — Урок 07 Фази 3**, звірити з ROADMAP.md перед стартом (ймовірно регуляризація DL — dropout/batch norm/early stopping, або RNN/sequence-моделі).

**Рішення від 2026-08-12/13 (Урок 06 Фази 3, технічна частина):** Перша повна CNN на MNIST — архітектура (`Conv→ReLU→Pool→Conv→ReLU→Pool→Flatten→Linear`), тренування, оцінка, порівняння з `Linear`-бейзлайном — усе технічно зроблено і працює. Результат: **CNN 98.04-98.17% accuracy / 5,258 параметрів, Linear MLP 96.63-97.09% / 101,770 параметрів** — CNN краща і майже в 19 разів економніша, пряме числове підтвердження weight sharing з Уроку 05. Другу модель (`LinearMLP`) і весь її training/eval цикл студент написав **повністю сам**, без підказок — сильний сигнал трансферу навички. Код: `code/phase-3/06_cnn_mnist.py`, drill-задачі: `code/phase-3/retrieval_weight_sharing.py`.
...
**Де зупинились:** Фаза 3 стартувала 09.08, **Уроки 01-09 пройдено (9/12)**. Урок 09 (Transformers — механіка self-attention) закрито 13.08, та сама сесія що Уроки 06-08 — дуже довгий марафон з чотирьох уроків поспіль. **Наступний крок — Урок 10 Фази 3, multi-head attention + практичний проект (Transformers), обов'язково окрема свіжа сесія** — не продовжувати марафон, тема найважча в Фазі 3. Лишається за планом далі: transfer learning, робота з GPU. Уроки 02-07 пройшли значно легше за Урок 01 (механіка/API vs математична семантика) — підтверджений патерн. Retention-перевірка "форвард vs backward pass" закрита (12.08); "epoch/batch nesting" і "tensor shape nesting" (структурний, не семантичний паттерн, 3+ підтвердження) — у черзі `voice-retrieval-queue.md`. Weight-sharing chain rule — стабілізувався 13.08 (третій прохід), контрольна перевірка через кілька уроків ще варта уваги. Dropout ("чому випадкові, не погані нейрони") і batch norm ("навіщо нормалізація") — нові пункти в `voice-retrieval-queue.md` з Уроку 07. Early stopping — перша тема Фази 3 де Feynman зійшовся з першої спроби, вартий контрольної перевірки чи це стійкий зсув паттерну. Уроку 06 Фази 2 (Gradient Boosting) і PCA-семантики (Урок 09 Фази 2) retention-check — досі бажаний, у черзі. **Дрон-проєкт "Колонія"** (`~/development/colony`, див. пам'ять) — потенційний реальний datasеt для CNN-блоку/Проєкту 2, звірити коли дійдемо туди.
...
- **Урок 06** — Перша повна CNN на MNIST: `Conv→ReLU→Pool→Conv→ReLU→Pool→Flatten→Linear`, тренування/оцінка на реальних 60k прикладах, порівняння з `Linear` baseline (CNN 98%+/5,258 параметрів vs Linear 97%/101,770) — числове підтвердження weight sharing на реальних даних ([docs](docs/phase-3-deep-learning/06-cnn-mnist.md), [memories](memories/lessons/04-06-cnn-mnist.md), [код](code/phase-3/06_cnn_mnist.py)).
- **Урок 07** — Регуляризація в DL: dropout (розміщення, чому не на фінальних logits), batch norm (`gamma`/`beta` по каналу, стабілізація масштабу), early stopping (`random_split`, validation vs test leakage, `patience`) — фінальний результат 98.82% test / 98.95% train, кращий за Урок 06 ([docs](docs/phase-3-deep-learning/07-regularization.md), [memories](memories/lessons/04-07-regularization.md), [код](code/phase-3/07_regularization.py)).
- **Урок 08** — RNN/LSTM (коротко за ROADMAP): hidden state на іграшковій задачі (сума послідовності), `dim`-семантика на 3D-тензорах, LSTM концептуально (vanishing gradient по часових кроках, gates) ([docs](docs/phase-3-deep-learning/08-rnn-lstm.md), [memories](memories/lessons/04-08-rnn-lstm.md), [код](code/phase-3/08_rnn_lstm.py)).
- **Урок 09** — Transformers механіка: Q/K/V, scaled dot-product attention (`softmax(QKᵀ/√d)V`), positional encoding, `SelfAttention` в PyTorch ([docs](docs/phase-3-deep-learning/09-attention.md), [memories](memories/lessons/04-09-attention.md), [код](code/phase-3/09_attention.py)).
